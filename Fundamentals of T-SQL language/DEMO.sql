...
DROP INDEX IF EXISTS [IX_Address_City] ON [Person].[Address];
...
--HAVING DATEPART(YEAR,EndDate) IS NOT NULL
...
-- DROP it if exists: 
DROP TABLE IF EXISTS [Sales].[Currencytest];
GO
...
DROP FUNCTION IF EXISTS [Test_Scalar_WorkOrder];
DROP FUNCTION IF EXISTS [TestInlineTF];
DROP FUNCTION IF EXISTS [TestTVF];
